...
private:
  ...
  
  {
    ...
    // Calculate A: 
    At_ = calculate_A(odom_dx, odom_dy);
  ...
  arma::mat calculate_K(arma::mat Sigma_bar, arma::mat Ht){
    // TODO: Pull out the R matrix to be a parameter.
  ...
  arma::mat calculate_H(arma::vec zi){
    ...
    for (int i = 0; i < int(zi.n_rows/2); i++){
      ...
      double delta_y = mu_bar_.at(4 + (2*i)) - mu_bar_.at(2);
  ...
  // TODO: Pull out the Q matrix to be a parameter.
  ...
  arma::mat calculate_A(double dx, double dy){
